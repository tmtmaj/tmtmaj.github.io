...
사실, 일반적으로 새로운 모듈을 위에 올리는 것은 새로운 테스크에 적용하기 위함입니다. 다른 데이터 셋이나 다른 목적으로 훈련된 A 모델도 새로운 모듈 B와 함께 새로운 데스크에 맞춰 약간 수정하는 것이 좋은 경우도 있습니다. (프리징을 하지 않고 A 모델 역시 B 모델과 함께(Jointly) 훈련)
